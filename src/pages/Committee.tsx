...
import { Check, X, RotateCcw, FileText, Download, Eye } from "lucide-react";
...
const Committee = () => {
  ...
  const handleViewDocument = (doc: { type: string; fileName: string }) => {
    ...
    <SidebarProvider>
      ...
      <Dialog open={!!selectedDocument} onOpenChange={() => setSelectedDocument(null)}>
        <DialogContent className="max-w-2xl max-h-[80vh]">
          ...
          <div className="flex flex-col items-center justify-center py-8 bg-muted/30 rounded-lg min-h-[300px]">
            <FileText className="h-16 w-16 text-muted-foreground mb-4" />
            <p className="text-lg font-medium">{selectedDocument?.fileName}</p>
          ...
          <DialogFooter className="flex gap-2">
            ...
            <Button 
              onClick={() => {
                if (selectedDocument) handleDownloadDocument(selectedDocument.fileName);
              }}
              className="gap-2"
            >
              <Download className="h-4 w-4" />
